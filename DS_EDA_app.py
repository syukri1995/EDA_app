        ...
        for i, col in enumerate(selected_columns):
            ...
            with plot_cols[i % cols_per_row]:
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    # ⚡ Bolt: Dynamically disable KDE for large datasets to prevent main-thread UI freezing
                    enable_kde = len(df) <= 50000
                    sns.histplot(df[col].dropna(), kde=enable_kde, ax=ax, palette='viridis')
                    title_suffix = 'Histogram/KDE' if enable_kde else 'Histogram'
