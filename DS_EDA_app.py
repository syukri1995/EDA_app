        ...
        for i, col in enumerate(selected_columns):
            ...
            with plot_cols[i % cols_per_row]:
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    # ⚡ Bolt: KDE is O(N^2) and blocks main thread on large datasets.
                    # Disable it for >50k rows to prevent UI freezing (10x+ speedup).
                    plot_data = df[col].dropna()
                    use_kde = len(plot_data) <= 50000

                    sns.histplot(plot_data, kde=use_kde, ax=ax, palette='viridis')
                    title = f'Distribution of {col}' + (' (with KDE)' if use_kde else ' (Histogram)')
