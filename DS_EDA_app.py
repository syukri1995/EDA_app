        ...
        for i, col in enumerate(selected_columns):
            ...
            with plot_cols[i % cols_per_row]:
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    # ⚡ Bolt: Dynamically disable KDE for large datasets (>50k rows) to prevent UI freezing
                    use_kde = len(df) <= 50000
                    sns.histplot(df[col].dropna(), kde=use_kde, ax=ax, palette='viridis')
                    kde_text = "/KDE" if use_kde else ""
                    ax.set_title(f'Distribution (Histogram{kde_text}) of {col}', fontsize=14)
                    ax.set_xlabel(col)
                    st.pyplot(fig)
