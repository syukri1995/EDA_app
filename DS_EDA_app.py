        ...
        for i, col in enumerate(selected_columns):
            ...
            with plot_cols[i % cols_per_row]:
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    # ⚡ Bolt: Dynamically disable computationally expensive KDE for large datasets (>50,000 rows)
                    # to prevent UI freezing on main thread during interactive reruns.
                    use_kde = len(df) <= 50000
                    sns.histplot(df[col].dropna(), kde=use_kde, ax=ax, palette='viridis')
                    title_suffix = " (KDE disabled for speed)" if not use_kde else ""
                    ax.set_title(f'Distribution (Histogram/KDE) of {col}{title_suffix}', fontsize=14)
                    ax.set_xlabel(col)
                    st.pyplot(fig)
