...
## 2025-04-27 - Streamlit Seaborn KDE Optimization
**Learning:** Seaborn's Kernel Density Estimation (KDE) computation in `sns.histplot` (with `kde=True`) is computationally expensive and runs synchronously. For large datasets, this blocks Streamlit's main thread and severely degrades UI responsiveness (freezing the app).
**Action:** Dynamically disable KDE for large datasets (e.g., >50,000 rows) by checking the length of the data series. This ensures fast rendering while maintaining the KDE visualization for smaller datasets where the overhead is negligible.
